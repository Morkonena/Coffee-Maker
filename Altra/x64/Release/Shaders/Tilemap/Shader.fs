...
float GetVertexLighting (int x, int y)
{
	...
	a = texture(lightmap, vec2(float(x)     / float(ve + 2), float(y) / float(ve + 2))).r;
	b = texture(lightmap, vec2(float(x + 1) / float(ve + 2), float(y) / float(ve + 2))).r;
	
	c = max(a, b);
	
	a = texture(lightmap, vec2(float(x)     / float(ve + 2), float(y + 1) / float(ve + 2))).r;
	b = texture(lightmap, vec2(float(x + 1) / float(ve + 2), float(y + 1) / float(ve + 2))).r;
...
void main ()
{
	...
	if (index > 0)
	{
		...
		color = texture(tileset, vec3(u, v, index - 1));
